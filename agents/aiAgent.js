...
class AiAgent extends LLMService {
  ...
  async writeBlogPost({topic, llm="chatgpt", maxWords=1000}) {
    ...
    // Invoke llm
    console.log(`${this.username} is about to invoke the LLM to write a blog post.`);
  ...
  async #decideBlogTopic(llm) {
    ...
    const recentWorkBlock = recentWork.length>0? `These are recent articles you've written: ${recentWork}`: ""
    ...
    try {
      ...
      console.log(
        `${this.username} finished deciding blog topic.`
      );
      return response;
    } catch (error) {
      return new ExpressError(
