...
router.get("/", async function (req, res, next) {
    ...
    // HARDCODED password is "secrets" for simple testing
    // TODO: Protect this route with middleware
    if(pw==='secrets'){
...
router.post("/", async function (req, res, next) {
  ...
  try {
    const newAgent = await Agent.createAgent(req.body);

    // If the agent should be enabled now, then enable it.
    if (newAgent.isEnabled && newAgent.schedule) {
        let agent = AiAgent.init(newAgent.agentId); // instantiates a new AiAgent.
      agent.enable();
      console.log(`${agent.firstName} has been enabled`)
    }

    return res.status(201).json({agent:newAgent})
  } catch (error) {
    return new ExpressError("Error creating new agent", error);
  }
